...
const headerTotals = headers.reduce(
  (sum, item) => {
    sum.amount += Number(item?.amount || 0);
    sum.vatAmount += Number(item?.vat_amount || 0);
    sum.totalAmount += Number(item?.total_amount || 0);
    return sum;
  },
  { amount: 0, vatAmount: 0, totalAmount: 0 }
);

const hasHeaderTotals = headers.length > 0;

const Total = hasHeaderTotals
  ? headerTotals.amount
  : Number(details?.amount || 0);

const TotalVAT = hasHeaderTotals
  ? headerTotals.vatAmount
  : Number(details?.vat_amount || 0);

const grand = hasHeaderTotals
  ? headerTotals.totalAmount
  : Number(details?.total_amount || 0);
