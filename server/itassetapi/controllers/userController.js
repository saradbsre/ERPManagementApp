...
exports.confirmReqest = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const { isApprove, email, activeUserEmail } = req.body;
    ...
    await pool.request()
      .input("id", sql.Int, id)
      .input("isApprove", sql.Bit, isApprove ? 1 : 0)
      .input("activeUserEmail", sql.NVarChar, activeUserEmail)
      .query(`
        UPDATE users 
        SET confirm = @isApprove,
            is_rejected = CASE WHEN @isApprove = 1 THEN 0 ELSE 1 END,
    ...
    await auditLogger({
      ...
      log_type: "INFO",
      module_name: "users",
      action: isApprove ? "APPROVE_SIGNUP_REQUEST" : "REJECT_SIGNUP_REQUEST",
      message: `${isApprove ? "Approved" : "Rejected"} signup request for user ID ${id}`,
      userid: activeUserEmail,
    });

    res.json({
      message: `User request is ${isApprove ? "approved" : "rejected"}`
    });
...
exports.toggleUserStatus = async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .query(`
        UPDATE users 
        SET is_active = @isActive, auditrev = ISNULL(auditrev, 0) + 1, userid = @activeUser, sysdate = GETDATE()
        WHERE id = @id
      `);
...
exports.togglePRFAccess = async (req, res) => {
  let pool;

  try {
    const { id } = req.params;
    const { prfaccess, email, activeUserEmail } = req.body;
    ...
    await pool.request()
      .input("id", sql.Int, id)
      .input("prfaccess", sql.Bit, prfaccess)
      .input("activeUser", sql.NVarChar, activeUser)
      .query(`
        UPDATE users 
        SET prf_access = @prfaccess, auditrev = ISNULL(auditrev, 0) + 1, userid = @activeUser, sysdate = GETDATE()
        WHERE id = @id
      `);
    ...
    await auditLogger({
      ...
      transaction_id: userEmail,   // ✅ IMPORTANT CHANGE
      module_name: "users",
      action: "TOGGLE_PRF_ACCESS",
      message: `User ${id} ${prfaccess ? "granted" : "revoked"} PRF access`,
      userid: activeUser,
    });

    res.json({ message: `User ${prfaccess ? "granted" : "revoked"} PRF access` });
    ...
    if (pool) {
      await auditLogger({
        ...
        module_name: "users",
        action: "TOGGLE_PRF_ACCESS",
        message: "Failed to toggle PRF access",
        status: "FAILED",
        error_message: err.message,
        userid: activeUser,
      });
...
exports.updateUserRole = async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .query(`
        UPDATE users 
        SET role = @role, auditrev = ISNULL(auditrev, 0) + 1, userid = @activeUserEmail, sysdate = GETDATE()
        WHERE id = @id
      `);
...
exports.signupRequests = async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
        SELECT id, name, email, phone_number, role, is_active, confirm, is_rejected
        FROM users
        WHERE confirm = 0
    `);
    res.json(result.recordset);
...
exports.acceptSignupRequest = async (req, res) => {
  try {
    ...
    await pool.request()
      ...
      .query(`
        UPDATE users
        SET confirm = 0, is_active = 1, sysdate = GETDATE()
        WHERE id = @id 
      `);
...
exports.forgetPassword = async (req, res) => {
  try {
    ...
    await pool.request()
      ...
      .query(`
        UPDATE users
        SET is_forget_password = 1, sysdate = GETDATE()
        WHERE email = @email
      `);
